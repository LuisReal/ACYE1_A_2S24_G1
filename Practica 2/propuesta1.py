...
bandera = True #While principal para sensores
...
def check_password_input():
    global Password_comparar, pattern_set, failed_attempts,bandera
    mylcd.lcd_clear()
    bandera = False
    while True:
...
def main():
    try:
        while bandera:
            sleep(1)  # Esperar 1 segundo en cada iteración
...
sensor_thread = threading.Thread(target=contra)
